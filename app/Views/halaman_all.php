    ...
    <div class="row">
      <div class="col-sm-8">
        ...
        <?php foreach ($halaman as $artikel): ?>
          <h2><a href="<?= base_url('home/halaman/'.$artikel['id']); ?>" style="text-decoration: none; color: black;" 
          onMouseOver="this.style.color='#00F'" onMouseOut="this.style.color='#000000'">
              <?= $artikel['judulhalaman']; ?></a>
          </h2>
          <h5><?= $artikel['author']; ?>, <?= $artikel['tglposting']; ?></h5>
          <p>Some text..</p>
          <p> <?= substr($artikel['isihalaman'], 0, 500)."......."; ?><a href="
              <?= base_url('home/halaman/'.$artikel['id']); ?>" style="text-decoration: none;">Baca Selengkapnya</a>
          </p>
          <br>
        <?php endforeach; ?>
        <?php endif; ?>
